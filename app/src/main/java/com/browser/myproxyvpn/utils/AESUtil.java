...
public class AESUtil {
    ...
    public static String encrypt(String key, String iv, String data) {
        try {
            if (key.length() < AESUtil.CIPHER_KEY_LEN) {
                int numPad = AESUtil.CIPHER_KEY_LEN - key.length();

                for(int i = 0; i < numPad; i++){
                    key += "0"; //0 pad to len 16 bytes
                }

            } else if (key.length() > AESUtil.CIPHER_KEY_LEN) {
            ...
            Cipher cipher = Cipher.getInstance(AESUtil.CIPHER_NAME);
